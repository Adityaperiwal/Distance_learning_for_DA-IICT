  ...
  export default {
    ...
    methods: {
      async getDetail () {
        ...
        console.log(jwt)
        if (jwt) {
          axios.get('http://192.168.137.1:3000/faculty/evaluation/student/detail/' + this.$route.params.student_id + '/' + this.$route.params.subtopic_id,
            {
              headers: {
      ...
      addCourses () {
        ...
        axios.patch('http://192.168.137.1:3000/faculty/evaluation/student/detail/' + ss_id, {subtopic_assignment_acquiredMarks: this.subtopic_assignment_acquiredMarks},
          {
            headers: {
          ...
          .then(r => console.log('r: ', JSON.stringify(r, null, 2)), this.$router.push('/faculty/index'), alert('Test Edit Successfully'))
          .catch(error => {
